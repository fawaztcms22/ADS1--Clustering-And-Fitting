...
def err_ranges(x, func, param, sigma):
    ...
    return lower, upper        
# In[28]:
# read file with China data into dataframe          
df_forest = pd.read_csv("Dataset Forest area Final.csv")
# have a look
print(df_forest)
# First fit attempt of the exponential function with defaul initial parameters
# In[29]:
# fit exponential growth
popt, covar = opt.curve_fit(exp_growth, df_forest["Years"], 
                            df_forest["China"])
# Calculate and plot the result
# In[30]:
print("Fit parameter", popt)
# use *popt to pass on the fit parameters
df_forest["pop_exp"] = exp_growth(df_forest["Years"], *popt)
plt.figure()
plt.plot(df_forest["Years"], df_forest["China"], label="data")
plt.plot(df_forest["Years"], df_forest["pop_exp"], label="fit")
plt.legend()
...
# Try scaling with the 1950 China and a smaller exponential factor
...
# China in 1990: about 800 million
# kept growth value from before
# increase scale factor and growth rate until rough fit
popt = [8e8, 0.02, 1990]
df_forest["pop_log"] = logistics(df_forest["Years"], *popt)
plt.figure()
plt.plot(df_forest["Years"], df_forest["China"], label="data")
plt.plot(df_forest["Years"], df_forest["pop_log"], label="fit")
plt.legend()
plt.xlabel("year")
plt.ylabel("China")
plt.title("Improved start value")
plt.show()
# In[34]:
popt, covar = opt.curve_fit(logistics, df_forest["Years"], df_forest["China"], 
                            p0=(2e9, 0.05, 1990.0))
print("Fit parameter", popt)
...
print(sigma)
low, up = err_ranges(df_forest["Years"], logistics, popt, sigma)
plt.figure()
plt.title("logistics function")
plt.plot(df_forest["Years"], df_forest["China"], label="data")
plt.plot(df_forest["Years"], df_forest["pop_log"], label="fit")
plt.fill_between(df_forest["Years"], low, up, alpha=0.7)
plt.legend()
plt.xlabel("year")
plt.ylabel("China")
plt.show()
